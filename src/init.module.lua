...
-- After each vote, refresh cached results so in-world boards (SurfaceGui) update without spamming clients with ResultsPublished.
...
-- ProfileService votes (DataStore) → in-memory store so tallies include all loaded profiles, not only session writes.
...
-- Union: cross-server DataStore ledger (authoritative) + session store for users not yet in ledger.
...
function ElectionManager:invalidateVoteByUserId(userId: number): string
	...
	Diagnostics.log(("CMD invalidate_vote userId=%s"):format(uidStr))
...
function ElectionManager:recordVote(player: Player, ballot: Types.Ballot): boolean
	...
	if not DistrictManager.isBallotAllowedForDistrict(ballot, district) then
		...
		Diagnostics.log(("VOTE denied user=%s phase=DISTRICT_MISMATCH"):format(player.Name))
