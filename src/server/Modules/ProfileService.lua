...
export type ProfileStore = {
	...
	LoadProfileAsync: (
		self: ProfileStore,
		profile_key: string,
		not_released_handler: "ForceLoad"
			| "Steal"
			| (place_id: number | nil, game_job_id: string | nil) -> "Repeat" | "Cancel" | "ForceLoad" | "Steal"
	) -> Profile | nil,
	GlobalUpdateProfileAsync: (
		self: ProfileStore,
		profile_key: string,
		update_handler: (profile: Profile) -> nil
	) -> GlobalUpdates | nil,
	ViewProfileAsync: (self: ProfileStore, profile_key: string, version: string | nil) -> nil,
	ProfileVersionQuery: (
		self: ProfileStore,
		profile_key: string,
		sort_direction: Enum.SortDirection | nil,
		min_date: DateTime | nil,
		max_date: DateTime | nil
	) -> ProfileVersionQuery,
}

export type ProfileVersionQuery = {
	NextPageAsync: (self: ProfileVersionQuery) -> Profile | nil,
}
...
export type Profile = {
	...
	GetMetaTag: (self: Profile, tag_name: string) -> any,
	Reconcile: (self: Profile) -> nil,
	ListenToRelease: (self: Profile, listener: (place_id: number | nil, game_job_id: number | nil) -> nil) -> RBXScriptSignal,
	Release: (self: Profile) -> nil,
	ListenToHopeReady: (self: Profile, listener: () -> nil) -> RBXScriptSignal,
	AddUserId: (self: Profile, user_id: number) -> nil,
	RemoveUserId: (self: Profile, user_id: number) -> nil,
	Identify: (self: Profile) -> string,
	SetMetaTag: (self: Profile, tag_name: string, value: any) -> nil,
	Save: (self: Profile) -> nil,

	ClearGlobalUpdates: (self: Profile) -> nil,
	OverwriteAsync: (self: Profile) -> nil,
}

export type GlobalUpdates = {
	GetActiveUpdates: (self: GlobalUpdates) -> table,
	GetLockedUpdates: (self: GlobalUpdates) -> table,

	ListenToNewActiveUpdate: (self: GlobalUpdates, listener: (update_id: number, update_data: table) -> nil) -> RBXScriptSignal,
	ListenToNewLockedUpdate: (self: GlobalUpdates, listener: (update_id: number, update_data: table) -> nil) -> RBXScriptSignal,
	LockActiveUpdate: (self: GlobalUpdates, update_id: number) -> nil,
	ClearLockedUpdate: (self: GlobalUpdates, update_id: number) -> nil,

	AddActiveUpdate: (self: GlobalUpdates, update_data: table) -> nil,
	ChangeActiveUpdate: (self: GlobalUpdates, update_id: number, update_data: table) -> nil,
	ClearActiveUpdate: (self: GlobalUpdates, update_id: number) -> nil,
}
...
local function RegisterCorruption(store_name, store_scope, profile_key) -- Called when a corrupted profile is loaded
	logger:AtWarning():Log("[ProfileService]: Resolved profile corruption " .. IdentifyProfile(store_name, store_scope, profile_key))
					...
					if is_get_call == true then
						...
						if version ~= nil then
							...
							if success == false and type(error_message) == "string" and string.find(error_message, "not valid") ~= nil then
								logger
									:AtWarning()
									:Log("[ProfileService]: Passed version argument is not valid; Traceback:\n" .. debug.traceback())
							end
						else
...
local function SaveProfileAsync(profile, release_from_session, is_overwriting)
	if type(profile.Data) ~= "table" then
		...
		logger:AtError():Log("[ProfileService]: PROFILE DATA CORRUPTED DURING RUNTIME! Profile: " .. profile:Identify())
	end
...
function GlobalUpdates:ListenToNewActiveUpdate(listener) --> [ScriptConnection] listener(update_id, update_data)
	if type(listener) ~= "function" then
		logger:AtError():Log("[ProfileService]: Only a function can be set as listener in GlobalUpdates:ListenToNewActiveUpdate()")
	end
	local profile = self._profile
	if self._update_handler_mode == true then
		logger:AtError():Log("[ProfileService]: Can't listen to new global updates in ProfileStore:GlobalUpdateProfileAsync()")
	elseif self._new_active_update_listeners == nil then
		logger:AtError():Log("[ProfileService]: Can't listen to new global updates in view mode")
...
function GlobalUpdates:ListenToNewLockedUpdate(listener) --> [ScriptConnection] listener(update_id, update_data)
	if type(listener) ~= "function" then
		logger:AtError():Log("[ProfileService]: Only a function can be set as listener in GlobalUpdates:ListenToNewLockedUpdate()")
	end
	local profile = self._profile
	if self._update_handler_mode == true then
		logger:AtError():Log("[ProfileService]: Can't listen to new global updates in ProfileStore:GlobalUpdateProfileAsync()")
...
function GlobalUpdates:LockActiveUpdate(update_id)
	if type(update_id) ~= "number" then
		logger:AtError():Log("[ProfileService]: Invalid update_id")
	end
	local profile = self._profile
	if self._update_handler_mode == true then
		logger:AtError():Log("[ProfileService]: Can't lock active global updates in ProfileStore:GlobalUpdateProfileAsync()")
	elseif self._pending_update_lock == nil then
		logger:AtError():Log("[ProfileService]: Can't lock active global updates in view mode")
	elseif profile:IsActive() == false then -- Check if profile is expired
		logger:AtError():Log("[ProfileService]: PROFILE EXPIRED - Can't lock active global updates")
	end
	...
	if global_update_exists ~= nil then
		...
		logger:AtError():Log("[ProfileService]: Passed non-existant update_id")
	end
end
...
function GlobalUpdates:ClearLockedUpdate(update_id)
	...
	local profile = self._profile
	if self._update_handler_mode == true then
		logger:AtError():Log("[ProfileService]: Can't clear locked global updates in ProfileStore:GlobalUpdateProfileAsync()")
	elseif self._pending_update_clear == nil then
		logger:AtError():Log("[ProfileService]: Can't clear locked global updates in view mode")
	elseif profile:IsActive() == false then -- Check if profile is expired
		logger:AtError():Log("[ProfileService]: PROFILE EXPIRED - Can't clear locked global updates")
	end
	-- Check if global update exists with given update_id
	local global_update_exists = nil
	for _, global_update in ipairs(self._updates_latest[2]) do
...
function GlobalUpdates:AddActiveUpdate(update_data)
	if type(update_data) ~= "table" then
		logger:AtError():Log("[ProfileService]: Invalid update_data")
	end
	if self._new_active_update_listeners ~= nil then
		logger
			:AtError()
			:Log("[ProfileService]: Can't add active global updates in loaded Profile; Use ProfileStore:GlobalUpdateProfileAsync()")
	elseif self._update_handler_mode ~= true then
		logger:AtError():Log("[ProfileService]: Can't add active global updates in view mode; Use ProfileStore:GlobalUpdateProfileAsync()")
	end
...
function GlobalUpdates:ChangeActiveUpdate(update_id, update_data)
	...
	if self._new_active_update_listeners ~= nil then
		logger
			:AtError()
			:Log("[ProfileService]: Can't change active global updates in loaded Profile; Use ProfileStore:GlobalUpdateProfileAsync()")
	elseif self._update_handler_mode ~= true then
		logger
			:AtError()
			:Log("[ProfileService]: Can't change active global updates in view mode; Use ProfileStore:GlobalUpdateProfileAsync()")
	end
	-- self._updates_latest = {}, -- [table] {update_index, {{update_id, version_id, update_locked, update_data}, ...}}
	local updates_latest = self._updates_latest
	...
	if get_global_update ~= nil then
		if get_global_update[3] == true then
			logger:AtError():Log("[ProfileService]: Can't change locked global update")
		end
...
function GlobalUpdates:ClearActiveUpdate(update_id)
	if type(update_id) ~= "number" then
		logger:AtError():Log("[ProfileService]: Invalid update_id argument")
	end
	if self._new_active_update_listeners ~= nil then
		logger
			:AtError()
			:Log("[ProfileService]: Can't clear active global updates in loaded Profile; Use ProfileStore:GlobalUpdateProfileAsync()")
	elseif self._update_handler_mode ~= true then
		logger
			:AtError()
			:Log("[ProfileService]: Can't clear active global updates in view mode; Use ProfileStore:GlobalUpdateProfileAsync()")
	end
	-- self._updates_latest = {}, -- [table] {update_index, {{update_id, version_id, update_locked, update_data}, ...}}
	local updates_latest = self._updates_latest
	...
	if get_global_update ~= nil then
		if get_global_update[3] == true then
			logger:AtError():Log("[ProfileService]: Can't clear locked global update")
		end
...
function Profile:GetMetaTag(tag_name) --> value
	...
	if meta_data == nil then
		return nil
		-- logger:AtError():Log("[ProfileService]: This Profile hasn't been loaded before - MetaData not available")
	end
...
function Profile:SetMetaTag(tag_name, value)
	if type(tag_name) ~= "string" then
		logger:AtError():Log("[ProfileService]: tag_name must be a string")
	elseif string.len(tag_name) == 0 then
		logger:AtError():Log("[ProfileService]: Invalid tag_name")
	end
...
function Profile:ListenToRelease(listener) --> [ScriptConnection] (place_id / nil, game_job_id / nil)
	if type(listener) ~= "function" then
		logger:AtError():Log("[ProfileService]: Only a function can be set as listener in Profile:ListenToRelease()")
	end
...
function Profile:Save()
	if self._view_mode == true then
		logger:AtError():Log("[ProfileService]: Can't save Profile in view mode - Should you be calling :OverwriteAsync() instead?")
	end
	if self:IsActive() == false then
		logger
			:AtWarning()
			:Log("[ProfileService]: Attempted saving an inactive profile " .. self:Identify() .. "; Traceback:\n" .. debug.traceback())
		return
	end
...
function Profile:ListenToHopReady(listener) --> [ScriptConnection] ()
	if type(listener) ~= "function" then
		logger:AtError():Log("[ProfileService]: Only a function can be set as listener in Profile:ListenToHopReady()")
	end
	if self._view_mode == true then
		return { Disconnect = function() end }
	end
...
function Profile:ClearGlobalUpdates() -- Clears all global updates data from a profile payload
	if self._view_mode ~= true then
		logger:AtError():Log("[ProfileService]: :ClearGlobalUpdates() can only be used in view mode")
	end
...
function Profile:OverwriteAsync() -- Saves the profile to the DataStore and removes the session lock
	if self._view_mode ~= true then
		logger:AtError():Log("[ProfileService]: :OverwriteAsync() can only be used in view mode")
	end
	...
	local function query_job()
		...
		if self._query_pages == nil then
			...
			if list_success == false or self._query_pages == nil then
				logger:AtWarning():Log("[ProfileService]: Version query fail - " .. tostring(error_message))
...
function ProfileStore:LoadProfileAsync(profile_key, not_released_handler, _use_mock) --> [Profile / nil] not_released_handler(place_id, game_job_id)
	...
	if self._profile_template == nil then
		logger:AtError():Log("[ProfileService]: Profile template not set - ProfileStore:LoadProfileAsync() locked for this ProfileStore")
	end
	if type(profile_key) ~= "string" then
		logger:AtError():Log("[ProfileService]: profile_key must be a string")
	elseif string.len(profile_key) == 0 then
		logger:AtError():Log("[ProfileService]: Invalid profile_key")
	end
	if type(not_released_handler) ~= "function" and not_released_handler ~= "ForceLoad" and not_released_handler ~= "Steal" then
		logger:AtError():Log("[ProfileService]: Invalid not_released_handler")
	end
...
function ProfileStore:GlobalUpdateProfileAsync(profile_key, update_handler, _use_mock) --> [GlobalUpdates / nil] (update_handler(GlobalUpdates))
	...
	if type(update_handler) ~= "function" then
		logger:AtError():Log("[ProfileService]: Invalid update_handler")
	end
...
function ProfileStore:ProfileVersionQuery(profile_key, sort_direction, min_date, max_date, _use_mock) --> [ProfileVersionQuery]
	...
	if _use_mock == UseMockTag or UseMockDataStore == true then
		logger:AtError():Log("[ProfileService]: :ProfileVersionQuery() is not supported in mock mode")
	end

	-- Type check:
	if sort_direction ~= nil and (typeof(sort_direction) ~= "EnumItem" or sort_direction.EnumType ~= Enum.SortDirection) then
		logger:AtError():Log("[ProfileService]: Invalid sort_direction (" .. tostring(sort_direction) .. ")")
	end

	if min_date ~= nil and typeof(min_date) ~= "DateTime" and typeof(min_date) ~= "number" then
		logger:AtError():Log("[ProfileService]: Invalid min_date (" .. tostring(min_date) .. ")")
	end

	if max_date ~= nil and typeof(max_date) ~= "DateTime" and typeof(max_date) ~= "number" then
		logger:AtError():Log("[ProfileService]: Invalid max_date (" .. tostring(max_date) .. ")")
	end
...
function ProfileService.GetProfileStore(profile_store_index, profile_template) --> [ProfileStore]
	...
	elseif type(profile_store_index) == "table" then
		...
		logger:AtError():Log("[ProfileService]: Invalid or missing profile_store_index")
	end

	-- Type checking:
	if profile_store_name == nil or type(profile_store_name) ~= "string" then
		logger:AtError():Log('[ProfileService]: Missing or invalid "Name" parameter')
	elseif string.len(profile_store_name) == 0 then
		logger:AtError():Log("[ProfileService]: ProfileStore name cannot be an empty string")
	end

	if profile_store_scope ~= nil and (type(profile_store_scope) ~= "string" or string.len(profile_store_scope) == 0) then
		logger:AtError():Log('[ProfileService]: Invalid "Scope" parameter')
	end

	if type(profile_template) ~= "table" then
		logger:AtError():Log("[ProfileService]: Invalid profile_template")
	end
...
if IsStudio == true then
	...
	task.spawn(function()
		...
		if no_internet_access == true then
			logger:AtWarning():Log("[ProfileService]: No internet access - check your network connection")
		end
		if
			status == false
			and (
		...
		then -- No internet access
			...
			logger:AtDebug():Log("[ProfileService]: Roblox API services unavailable - data will not be saved")
		else
			logger:AtDebug():Log("[ProfileService]: Roblox API services available - data will be saved")
		end
...
RunService.Heartbeat:Connect(function()
	...
	if ProfileService.CriticalState == false then
		if #IssueQueue >= SETTINGS.IssueCountForCriticalState then
			...
			logger:AtWarning():Log("[ProfileService]: Entered critical state")
		end
	else
		if #IssueQueue >= SETTINGS.IssueCountForCriticalState then
			CriticalStateStart = os.clock()
		elseif os.clock() - CriticalStateStart > SETTINGS.CriticalStateLast then
			...
			logger:AtWarning():Log("[ProfileService]: Critical state ended")
		end
	end
	-- Issue queue:
	while true do
